describe('Navigation Tests', () => {
  ...
  beforeEach(() => {
    cy.visit('/') //cy.visit(Cypress.env()['baseUrl'])
  })

  it('Validate Home page when application opens', () => {
    ...
    cy.log('Environment: ' + Cypress.env()['inputParam1'])
  })

  it('Validate navigation to Command Line Editor', () => {
  ...
  it('Validate navigation to Molecular Weight', () => {
  ...
  it('Validate navigation to Batch Model & Change', () => {

    cy.allure().feature('Navigation').testID('NT-04').owner('albertolop@gmail.com').tag('regression', 'navigation')

    cy.get('[test-id="show-menu-button"]').click()
    cy.get('[test-id="menu-option-batch"]').should('be.visible').and('have.text', 'Batch Model & Change').click()
